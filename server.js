...
/* ---------- CENTRALIZED MULTI-PROJECT EXTENSION ---------- */
function ensureCentralData(){
  db.contractors ||= [];
  db.materials ||= [];
  db.budgets ||= [];
  db.approvals ||= [];
  db.workflows ||= [];
  db.resource_allocations ||= [];

  const firstProject=db.projects[0]?.id;
  const secondProject=db.projects[1]?.id;
  const managerIds=db.users.filter(u=>u.role==='site_manager').map(u=>u.id);

  if(!db.contractors.length){
    db.contractors.push(
      {id:id(),project_id:firstProject,name:'Vertex Civil Works',trade:'Civil & Structure',contact:'+91 98765 10001',status:'Active',contract_value:1850000},
      {id:id(),project_id:firstProject,name:'Spark Electricals',trade:'Electrical',contact:'+91 98765 10002',status:'Active',contract_value:720000},
      {id:id(),project_id:secondProject,name:'Apex Finishes',trade:'Painting & Interiors',contact:'+91 98765 10003',status:'Active',contract_value:950000}
    );
  }
  if(!db.materials.length){
    db.materials.push(
      {id:id(),project_id:firstProject,name:'Cement',category:'Structural',unit:'bags',required:1800,received:1650,used:1420,reorder_level:250,status:'Available'},
      {id:id(),project_id:firstProject,name:'Electrical cable',category:'Electrical',unit:'m',required:6200,received:4100,used:3500,reorder_level:700,status:'Low Stock'},
      {id:id(),project_id:secondProject,name:'Premium tiles',category:'Finishes',unit:'boxes',required:900,received:620,used:510,reorder_level:120,status:'Low Stock'}
    );
  }
  if(!db.budgets.length){
    db.budgets.push(
      {id:id(),project_id:firstProject,category:'Civil Works',planned:4500000,spent:2980000,committed:620000},
      {id:id(),project_id:firstProject,category:'Electrical',planned:1200000,spent:650000,committed:180000},
      {id:id(),project_id:firstProject,category:'Materials',planned:2100000,spent:1320000,committed:260000},
      {id:id(),project_id:secondProject,category:'Civil Works',planned:5200000,spent:3150000,committed:740000},
      {id:id(),project_id:secondProject,category:'Finishes',planned:1800000,spent:910000,committed:240000}
    );
  }
  if(!db.approvals.length){
    db.approvals.push(
      {id:id(),project_id:firstProject,type:'Material Approval',title:'Electrical cable brand approval',requester_id:managerIds[0],status:'Pending',amount:0,notes:'Approve alternate cable brand for current procurement.',created_at:iso()},
      {id:id(),project_id:firstProject,type:'Budget Approval',title:'Additional testing allowance',requester_id:managerIds[0],status:'Approved',amount:85000,notes:'Additional allowance for electrical testing.',created_at:iso()},
      {id:id(),project_id:secondProject,type:'Procurement Approval',title:'Premium tile shipment',requester_id:managerIds[1]||managerIds[0],status:'Pending',amount:240000,notes:'Release balance payment to avoid delivery delay.',created_at:iso()}
    );
  }
  if(!db.workflows.length){
    db.projects.forEach((p,idx)=>{
      const names=idx===1
        ? ['Site Mobilization','Foundation','Structure','Walls','Roofing','Plumbing','Electrical','Painting','Interiors','Final Inspection','Handover']
        : ['Site Mobilization','Foundation','Structure','Walls','Cladding','Plumbing','Electrical','Painting','Flooring','Interiors','Final Inspection','Handover'];
      names.forEach((name,i)=>db.workflows.push({id:id(),project_id:p.id,name,seq:i+1,owner_role:i<4?'site_manager':'site_manager',required:true}));
    });
  }
  save();
}
...
function centralProjectAccess(req, projectId){
  const p=find('projects',projectId);
  if(!p) return false;
  if(req.session.user.role==='admin') return true;
  if(req.session.user.role==='site_manager') return p.manager_id===req.session.user.id;
  return db.units.some(u=>u.project_id===p.id && u.customer_id===req.session.user.id);
}
function visibleCentralProjects(req){
  return db.projects.filter(p=>centralProjectAccess(req,p.id));
}
...
app.get('/api/central/summary',auth,role('admin','site_manager'),(req,res)=>{
  const projects=visibleCentralProjects(req);
  const pids=new Set(projects.map(p=>p.id));
  const contractors=db.contractors.filter(x=>pids.has(x.project_id));
  const materials=db.materials.filter(x=>pids.has(x.project_id));
  const budgets=db.budgets.filter(x=>pids.has(x.project_id));
  const approvals=db.approvals.filter(x=>pids.has(x.project_id));
  const allocations=db.resource_allocations.filter(x=>pids.has(x.project_id));
  const planned=budgets.reduce((a,x)=>a+Number(x.planned||0),0);
  const spent=budgets.reduce((a,x)=>a+Number(x.spent||0),0);
  res.json({
    projects:projects.length,
    contractors:contractors.length,
    materials:materials.length,
    low_stock:materials.filter(x=>x.status==='Low Stock'||Number(x.received-x.used)<=Number(x.reorder_level||0)).length,
    budget_planned:planned,budget_spent:spent,budget_remaining:Math.max(0,planned-spent),
    pending_approvals:approvals.filter(x=>x.status==='Pending'||x.status==='In Review').length,
    active_resources:allocations.filter(x=>x.status==='Allocated').length
  });
});

app.get('/api/contractors',auth,role('admin','site_manager'),(req,res)=>{
  const pids=new Set(visibleCentralProjects(req).map(p=>p.id));
  res.json(db.contractors.filter(x=>pids.has(x.project_id)).map(x=>({...x,project_name:find('projects',x.project_id)?.name})));
});
app.post('/api/contractors',auth,role('admin'),(req,res)=>{
  const p=find('projects',req.body.project_id);
  if(!p)return res.status(400).json({error:'Project not found'});
  const c={id:id(),project_id:p.id,name:req.body.name,trade:req.body.trade,contact:req.body.contact||'',status:req.body.status||'Active',contract_value:Number(req.body.contract_value)||0};
  db.contractors.push(c);save();res.json({id:c.id});
});

app.get('/api/materials',auth,role('admin','site_manager'),(req,res)=>{
  const pids=new Set(visibleCentralProjects(req).map(p=>p.id));
  res.json(db.materials.filter(x=>pids.has(x.project_id)).map(x=>({...x,project_name:find('projects',x.project_id)?.name,available:Number(x.received||0)-Number(x.used||0)})));
});
app.post('/api/materials',auth,role('admin','site_manager'),(req,res)=>{
  const p=find('projects',req.body.project_id);
  if(!p||!centralProjectAccess(req,p.id))return res.status(403).json({error:'Access denied'});
  const m={id:id(),project_id:p.id,name:req.body.name,category:req.body.category||'General',unit:req.body.unit||'units',required:Number(req.body.required)||0,received:Number(req.body.received)||0,used:Number(req.body.used)||0,reorder_level:Number(req.body.reorder_level)||0,status:'Available'};
  m.status=(m.received-m.used)<=m.reorder_level?'Low Stock':'Available';
  db.materials.push(m);save();res.json({id:m.id});
});
app.post('/api/materials/:id/receive',auth,role('admin','site_manager'),(req,res)=>{
  const m=find('materials',req.params.id);
  if(!m||!centralProjectAccess(req,m.project_id))return res.status(404).json({error:'Material not found'});
  m.received+=Number(req.body.quantity)||0;
  m.status=(m.received-m.used)<=m.reorder_level?'Low Stock':'Available';
  save();res.json({ok:true,available:m.received-m.used,status:m.status});
});

app.get('/api/budgets',auth,role('admin','site_manager'),(req,res)=>{
  const pids=new Set(visibleCentralProjects(req).map(p=>p.id));
  res.json(db.budgets.filter(x=>pids.has(x.project_id)).map(x=>({...x,project_name:find('projects',x.project_id)?.name,remaining:Number(x.planned||0)-Number(x.spent||0)-Number(x.committed||0)})));
});
app.post('/api/budgets',auth,role('admin'),(req,res)=>{
  const p=find('projects',req.body.project_id);
  if(!p)return res.status(400).json({error:'Project not found'});
  db.budgets.push({id:id(),project_id:p.id,category:req.body.category||'General',planned:Number(req.body.planned)||0,spent:0,committed:0});
  save();res.json({ok:true});
});

app.get('/api/approvals',auth,role('admin','site_manager'),(req,res)=>{
  const pids=new Set(visibleCentralProjects(req).map(p=>p.id));
  res.json(db.approvals.filter(x=>pids.has(x.project_id)).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)).map(x=>({...x,project_name:find('projects',x.project_id)?.name,requester_name:find('users',x.requester_id)?.name})));
});
app.post('/api/approvals',auth,role('admin','site_manager'),(req,res)=>{
  const p=find('projects',req.body.project_id);
  if(!p||!centralProjectAccess(req,p.id))return res.status(403).json({error:'Access denied'});
  const a={id:id(),project_id:p.id,type:req.body.type||'General',title:req.body.title,requester_id:req.session.user.id,status:'Pending',amount:Number(req.body.amount)||0,notes:req.body.notes||'',created_at:iso()};
  db.approvals.push(a);save();res.json({id:a.id});
});
app.post('/api/approvals/:id/status',auth,role('admin','site_manager'),(req,res)=>{
  const a=find('approvals',req.params.id);
  if(!a||!centralProjectAccess(req,a.project_id))return res.status(404).json({error:'Approval not found'});
  if(!['Pending','In Review','Approved','Rejected'].includes(req.body.status))return res.status(400).json({error:'Invalid approval status'});
  a.status=req.body.status;save();res.json({ok:true});
});

app.get('/api/workflows',auth,role('admin','site_manager'),(req,res)=>{
  const pids=new Set(visibleCentralProjects(req).map(p=>p.id));
  const out=db.projects.filter(p=>pids.has(p.id)).map(p=>({...p,workflow:db.workflows.filter(w=>w.project_id===p.id).sort((a,b)=>a.seq-b.seq)}));
  res.json(out);
});
app.post('/api/workflows/:projectId/stages',auth,role('admin'),(req,res)=>{
  const p=find('projects',req.params.projectId);
  if(!p)return res.status(404).json({error:'Project not found'});
  const stages=db.workflows.filter(w=>w.project_id===p.id);
  const stage={id:id(),project_id:p.id,name:req.body.name,seq:stages.length+1,owner_role:req.body.owner_role||'site_manager',required:req.body.required!==false};
  db.workflows.push(stage);save();res.json({id:stage.id});
});
app.post('/api/resource-allocations',auth,role('admin','site_manager'),(req,res)=>{
  const p=find('projects',req.body.project_id);
  if(!p||!centralProjectAccess(req,p.id))return res.status(403).json({error:'Access denied'});
  const r={id:id(),project_id:p.id,type:req.body.type||'Equipment',name:req.body.name,quantity:Number(req.body.quantity)||1,unit:req.body.unit||'units',status:'Allocated',start_date:req.body.start_date||null,end_date:req.body.end_date||null};
  db.resource_allocations.push(r);save();res.json({id:r.id});
});
app.get('/api/resource-allocations',auth,role('admin','site_manager'),(req,res)=>{
  const pids=new Set(visibleCentralProjects(req).map(p=>p.id));
  res.json(db.resource_allocations.filter(x=>pids.has(x.project_id)).map(x=>({...x,project_name:find('projects',x.project_id)?.name})));
});



app.get('/api/report/export',auth,role('admin'),(req,res)=>{
  const rows=db.projects.map(p=>{
    const us=db.units.filter(u=>u.project_id===p.id);
    return [p.name,p.type,p.location,us.length,us.length?Math.round(us.reduce((a,u)=>a+u.progress,0)/us.length):0,us.filter(u=>['Ready Soon','Ready for Handover'].includes(u.status)).length,us.length?us.map(u=>recalcETA(u)).sort()[0]:'',us.length?us.map(u=>recalcETA(u)).sort().slice(-1)[0]:''];
  });
  const csv=[['Project','Type','Location','Homes','Average Progress','Ready','Earliest Delivery','Latest Delivery'],...rows]
    .map(r=>r.map(v=>`"${String(v??'').replace(/"/g,'""')}"`).join(',')).join('\n');
  res.setHeader('Content-Type','text/csv;charset=utf-8');
  res.setHeader('Content-Disposition','attachment; filename="buildtrack-report.csv"');
  res.send(csv);
});
