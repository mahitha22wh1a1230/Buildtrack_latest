...
function layout(content){
  ...
  const nav=u.role==='admin'
    ?[['dashboard','Dashboard','▦'],['projects','Projects','▣'],['homes','Homes','⌂'],['updates','Daily Updates','↻'],['resources','Resources & Budget','◈'],['workflows','Workflows','⌘'],['custom','Customizations','✦'],['issues','Issues & Risks','!'],['tasks','Tasks','✓'],['reports','Reports','▤'],['team','Team','♙']]
    :u.role==='site_manager'
    ?[['dashboard','Site Dashboard','▦'],['homes','My Site Homes','⌂'],['updates','Daily Updates','↻'],['resources','Resources','◈'],['workflows','Workflows','⌘'],['tasks','Tasks','✓'],['issues','Issues & Risks','!']]
...
async function dashboard(){
  ...
  let c=null;
  if(['admin','site_manager'].includes(state.user.role)){
    try{c=await api('/api/central/summary');state.central=c}catch(e){c=null}
  }
  const central=c?`
  <div style="height:18px"></div>
  <div class="section-title"><h2>Centralized Operations</h2><span class="small-note">Across all projects you can access</span></div>
  <div class="grid grid4 central-kpis">
    <div class="card stat"><div><div class="label">Contractors</div><h3>${c.contractors}</h3><div class="small-note">Active project partners</div></div><div class="iconbox">♙</div></div>
    <div class="card stat"><div><div class="label">Low-stock materials</div><h3>${c.low_stock}</h3><div class="small-note">Needs procurement attention</div></div><div class="iconbox">▤</div></div>
    <div class="card stat"><div><div class="label">Budget remaining</div><h3>${money(c.budget_remaining)}</h3><div class="small-note">Planned minus spent</div></div><div class="iconbox">₹</div></div>
    <div class="card stat"><div><div class="label">Pending approvals</div><h3>${c.pending_approvals}</h3><div class="small-note">Awaiting decision</div></div><div class="iconbox">✓</div></div>
  </div>
  <div style="height:18px"></div>
  <div class="grid grid2">
    <div class="card"><div class="section-title"><h2>Resource allocation</h2><button class="btn small" onclick="state.page='resources';load()">Manage</button></div>
      <div class="small-note">The centralized system keeps contractors, materials, budgets and resources visible across sites.</div>
      <div class="progress-row"><span>Projects</span><div class="progress"><span style="width:${Math.min(100,c.projects*25)}%"></span></div><b>${c.projects}</b></div>
      <div class="progress-row"><span>Resources</span><div class="progress"><span style="width:${Math.min(100,c.active_resources*20)}%"></span></div><b>${c.active_resources}</b></div>
    </div>
    <div class="card"><div class="section-title"><h2>Project-specific workflows</h2><button class="btn small" onclick="state.page='workflows';load()">Open</button></div>
      <p class="muted">Each project can have its own sequence of stages, owners and required steps.</p>
      <div class="check"><span>Projects with workflows</span><b>${c.projects}</b></div>
    </div>
  </div>`:'';
  return `
  ${statCards(d)}
...
function projectOptions(){
  return state.projects.map(p=>`<option value="${p.id}">${esc(p.name)}</option>`).join('');
}
async function resourcesPage(){
  const [contractors,materials,budgets,approvals,allocations]=await Promise.all([
    api('/api/contractors'),api('/api/materials'),api('/api/budgets'),api('/api/approvals'),api('/api/resource-allocations')
  ]);
  const budgetTotal=budgets.reduce((a,b)=>a+Number(b.planned||0),0);
  const spentTotal=budgets.reduce((a,b)=>a+Number(b.spent||0),0);
  return `<div class="page-head"><div><h1>Resources, Contractors & Budget</h1><div class="muted">Centralized visibility of people, materials, money and shared resources across construction sites.</div></div>
    <div class="toolbar">${state.user.role==='admin'?'<button class="btn primary" onclick="openContractor()">+ Contractor</button>':''}<button class="btn" onclick="openMaterial()">+ Material</button><button class="btn" onclick="openResource()">+ Resource</button><button class="btn gold" onclick="openApproval()">+ Approval</button></div></div>
    <div class="grid grid4">
      <div class="card stat"><div><div class="label">Contractors</div><h3>${contractors.length}</h3></div><div class="iconbox">♙</div></div>
      <div class="card stat"><div><div class="label">Materials</div><h3>${materials.length}</h3><div class="small-note">${materials.filter(m=>m.status==='Low Stock').length} low stock</div></div><div class="iconbox">▤</div></div>
      <div class="card stat"><div><div class="label">Planned budget</div><h3>${money(budgetTotal)}</h3></div><div class="iconbox">₹</div></div>
      <div class="card stat"><div><div class="label">Spent</div><h3>${money(spentTotal)}</h3><div class="small-note">${budgetTotal?Math.round(spentTotal/budgetTotal*100):0}% of plan</div></div><div class="iconbox">↗</div></div>
    </div>
    <div style="height:18px"></div>
    <div class="two-col">
      <div class="card"><div class="section-title"><h2>Contractors by project</h2></div>
        <div class="table-wrap"><table class="table"><thead><tr><th>Contractor</th><th>Project</th><th>Trade</th><th>Contact</th><th>Contract value</th><th>Status</th></tr></thead><tbody>
        ${contractors.map(c=>`<tr><td><b>${esc(c.name)}</b></td><td>${esc(c.project_name)}</td><td>${esc(c.trade)}</td><td>${esc(c.contact)}</td><td>${money(c.contract_value)}</td><td><span class="badge green">${esc(c.status)}</span></td></tr>`).join('')}</tbody></table></div>
      </div>
      <div class="card"><div class="section-title"><h2>Shared resources</h2></div>
        ${allocations.length?allocations.map(r=>`<div class="check"><div><b>${esc(r.name)}</b><div class="small-note">${esc(r.type)} · ${esc(r.project_name)}</div></div><span>${r.quantity} ${esc(r.unit)}</span></div>`).join(''):'<div class="empty">No resources allocated yet.</div>'}
      </div>
    </div>
    <div style="height:18px"></div>
    <div class="two-col">
      <div class="card"><div class="section-title"><h2>Material requirements</h2></div>
        <div class="table-wrap"><table class="table"><thead><tr><th>Material</th><th>Project</th><th>Required</th><th>Available</th><th>Status</th><th></th></tr></thead><tbody>
        ${materials.map(m=>`<tr><td><b>${esc(m.name)}</b><div class="small-note">${esc(m.category)}</div></td><td>${esc(m.project_name)}</td><td>${m.required} ${esc(m.unit)}</td><td>${m.available} ${esc(m.unit)}</td><td><span class="badge ${m.status==='Low Stock'?'orange':'green'}">${esc(m.status)}</span></td><td><button class="btn small" onclick="receiveMaterial(${m.id})">Receive</button></td></tr>`).join('')}</tbody></table></div>
      </div>
      <div class="card"><div class="section-title"><h2>Budget control</h2></div>
        ${budgets.map(b=>`<div class="budget-line"><div class="project-meta"><span><b>${esc(b.category)}</b> · ${esc(b.project_name)}</span><span>${money(b.spent)} / ${money(b.planned)}</span></div><div class="progress"><span style="width:${Math.min(100,b.planned?b.spent/b.planned*100:0)}%"></span></div><div class="small-note">Committed ${money(b.committed)} · Remaining ${money(b.remaining)}</div></div>`).join('')}
        ${state.user.role==='admin'?'<button class="btn small" onclick="openBudget()" style="margin-top:12px">+ Budget line</button>':''}
      </div>
    </div>
    <div style="height:18px"></div>
    <div class="card"><div class="section-title"><h2>Approvals</h2><span class="small-note">Material, procurement and budget decisions</span></div>
      <div class="table-wrap"><table class="table"><thead><tr><th>Request</th><th>Project</th><th>Type</th><th>Requester</th><th>Amount</th><th>Status</th><th>Action</th></tr></thead><tbody>
      ${approvals.map(a=>`<tr><td><b>${esc(a.title)}</b><div class="small-note">${esc(a.notes)}</div></td><td>${esc(a.project_name)}</td><td>${esc(a.type)}</td><td>${esc(a.requester_name)}</td><td>${a.amount?money(a.amount):'—'}</td><td><span class="badge ${a.status==='Approved'?'green':a.status==='Rejected'?'red':'orange'}">${esc(a.status)}</span></td><td>${a.status!=='Approved'&&a.status!=='Rejected'?`<button class="btn small" onclick="approvalStatus(${a.id},'Approved')">Approve</button> <button class="btn small" onclick="approvalStatus(${a.id},'Rejected')">Reject</button>`:'—'}</td></tr>`).join('')}</tbody></table></div>
    </div>`;
}

async function workflowsPage(){
  const rows=await api('/api/workflows');
  return `<div class="page-head"><div><h1>Project-Specific Workflows</h1><div class="muted">Each construction project can follow its own stages, owners and required checkpoints.</div></div></div>
  <div class="grid grid2">${rows.map(p=>`<div class="card workflow-card"><div class="section-title"><div><h2>${esc(p.name)}</h2><div class="small-note">${esc(p.type)} · ${esc(p.location||'')}</div></div>${state.user.role==='admin'?`<button class="btn small" onclick="openWorkflowStage(${p.id})">+ Stage</button>`:''}</div>
    <div class="workflow-list">${p.workflow.map((w,i)=>`<div class="workflow-step"><span class="workflow-number">${i+1}</span><div><b>${esc(w.name)}</b><div class="small-note">Owner: ${roleName(w.owner_role)} · ${w.required?'Required':'Optional'}</div></div></div>`).join('')}</div>
  </div>`).join('')}</div>`;
}

function openContractor(){
  openModal(`<div class="modal-head"><h2 style="margin:0">Add contractor</h2><button class="close" onclick="closeModal()">×</button></div><form id="contractorForm"><div class="form-grid"><div class="field full"><label>Project</label><select name="project_id">${projectOptions()}</select></div><div class="field"><label>Company / Contractor</label><input name="name" required></div><div class="field"><label>Trade</label><input name="trade" placeholder="Civil, Electrical, Plumbing" required></div><div class="field"><label>Contact</label><input name="contact"></div><div class="field"><label>Contract value</label><input type="number" name="contract_value" min="0"></div></div><button class="btn primary" style="margin-top:16px">Add contractor</button></form>`);
  $('#contractorForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);try{await api('/api/contractors',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.fromEntries(f))});closeModal();toast('Contractor added');await load()}catch(err){toast(err.message)}};
}
function openMaterial(){
  openModal(`<div class="modal-head"><h2 style="margin:0">Add material requirement</h2><button class="close" onclick="closeModal()">×</button></div><form id="materialForm"><div class="form-grid"><div class="field full"><label>Project</label><select name="project_id">${projectOptions()}</select></div><div class="field"><label>Material</label><input name="name" required></div><div class="field"><label>Category</label><input name="category" value="General"></div><div class="field"><label>Unit</label><input name="unit" value="units"></div><div class="field"><label>Required quantity</label><input type="number" name="required" min="0"></div><div class="field"><label>Received quantity</label><input type="number" name="received" min="0"></div><div class="field"><label>Reorder level</label><input type="number" name="reorder_level" min="0"></div></div><button class="btn primary" style="margin-top:16px">Add material</button></form>`);
  $('#materialForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);try{await api('/api/materials',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.fromEntries(f))});closeModal();toast('Material added');await load()}catch(err){toast(err.message)}};
}
async function receiveMaterial(id){
  const q=prompt('How many units/bags/boxes were received?','100'); if(q===null)return;
  try{await api('/api/materials/'+id+'/receive',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({quantity:Number(q)})});toast('Material stock updated');await load()}catch(e){toast(e.message)}
}
function openBudget(){
  openModal(`<div class="modal-head"><h2 style="margin:0">Add budget line</h2><button class="close" onclick="closeModal()">×</button></div><form id="budgetForm"><div class="field"><label>Project</label><select name="project_id">${projectOptions()}</select></div><div class="field" style="margin-top:12px"><label>Category</label><input name="category" required></div><div class="field" style="margin-top:12px"><label>Planned amount</label><input type="number" name="planned" min="0" required></div><button class="btn primary" style="margin-top:16px">Add budget line</button></form>`);
  $('#budgetForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);try{await api('/api/budgets',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.fromEntries(f))});closeModal();toast('Budget line added');await load()}catch(err){toast(err.message)}};
}
function openApproval(){
  openModal(`<div class="modal-head"><h2 style="margin:0">Create approval request</h2><button class="close" onclick="closeModal()">×</button></div><form id="approvalForm"><div class="form-grid"><div class="field"><label>Project</label><select name="project_id">${projectOptions()}</select></div><div class="field"><label>Type</label><select name="type"><option>Material Approval</option><option>Procurement Approval</option><option>Budget Approval</option><option>Workflow Approval</option></select></div><div class="field full"><label>Request title</label><input name="title" required></div><div class="field"><label>Amount</label><input type="number" name="amount" min="0"></div><div class="field full"><label>Notes</label><textarea name="notes"></textarea></div></div><button class="btn primary" style="margin-top:16px">Submit approval</button></form>`);
  $('#approvalForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);try{await api('/api/approvals',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.fromEntries(f))});closeModal();toast('Approval request created');await load()}catch(err){toast(err.message)}};
}
function approvalStatus(id,status){
  api('/api/approvals/'+id+'/status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({status})}).then(()=>{toast('Approval updated');load()}).catch(e=>toast(e.message));
}
function openResource(){
  openModal(`<div class="modal-head"><h2 style="margin:0">Allocate shared resource</h2><button class="close" onclick="closeModal()">×</button></div><form id="resourceForm"><div class="form-grid"><div class="field"><label>Project</label><select name="project_id">${projectOptions()}</select></div><div class="field"><label>Type</label><select name="type"><option>Equipment</option><option>Manpower</option><option>Vehicle</option><option>Site Office</option></select></div><div class="field"><label>Resource name</label><input name="name" required placeholder="Crane / Excavator / Crew"></div><div class="field"><label>Quantity</label><input type="number" name="quantity" min="1" value="1"></div><div class="field"><label>Unit</label><input name="unit" value="units"></div><div class="field"><label>Start date</label><input type="date" name="start_date"></div><div class="field"><label>End date</label><input type="date" name="end_date"></div></div><button class="btn primary" style="margin-top:16px">Allocate resource</button></form>`);
  $('#resourceForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);try{await api('/api/resource-allocations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.fromEntries(f))});closeModal();toast('Resource allocated');await load()}catch(err){toast(err.message)}};
}
function openWorkflowStage(projectId){
  openModal(`<div class="modal-head"><h2 style="margin:0">Add workflow stage</h2><button class="close" onclick="closeModal()">×</button></div><form id="workflowForm"><div class="field"><label>Stage name</label><input name="name" required placeholder="Procurement approval"></div><div class="field" style="margin-top:12px"><label>Owner</label><select name="owner_role"><option value="site_manager">Site Manager</option><option value="admin">Admin</option></select></div><label style="display:flex;gap:8px;align-items:center;margin-top:12px"><input type="checkbox" name="required" checked> Required checkpoint</label><button class="btn primary" style="margin-top:16px">Add stage</button></form>`);
  $('#workflowForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);try{await api('/api/workflows/'+projectId+'/stages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:f.get('name'),owner_role:f.get('owner_role'),required:f.get('required')==='on'})});closeModal();toast('Workflow stage added');await load()}catch(err){toast(err.message)}};
}
...
async function render(){
  ...
  try{
    ...
    else if(state.page==='resources')content=await resourcesPage();
    else if(state.page==='workflows')content=await workflowsPage();
